...
// ===== BOT & RL TEMELLERİ =====
let gameMode = "pvp"; // pvp | bot
let botColor = "sari"; // bot hangi renk oynuyor

let Q = JSON.parse(localStorage.getItem("Q_TABLE")) || {};

let alpha = 0.3;   // öğrenme hızı
let epsilon = 0.3; // keşif oranı (sonra zorluk ayarı yapacağız)
...
function switchTurn() {
  ...
  // BOT SIRASI MI?
  if (gameMode === "bot" && currentPlayer === botColor) {
...
function encodeState() {
  return board.flat().map(cell => {
    if (!cell) return 0;
    if (cell.color === "mor") return 1;
    return -1;
  }).join(",");
}


function encodeAction(sr, sc, tr, tc) {
  return `${sr},${sc}->${tr},${tc}`;
}
...
console.log(getAllPossibleMoves("mor"));
function botPlay() {
  const moves = getAllPossibleMoves(botColor);
  let bestMove = null;
  let bestScore = -Infinity;

  for (let move of moves) {
    // 1️⃣ BOT HAMLESİ
    const boardAfterBot = cloneBoard(board);
    simulateMove(boardAfterBot, move);

    const botGain =
      countStones(board, opponent(botColor)) -
      countStones(boardAfterBot, opponent(botColor));

    // 2️⃣ RAKİBİN EN İYİ CEVABI
    let worstEnemyGain = 0;
    const enemyMoves = getAllPossibleMoves(opponent(botColor));

    for (let em of enemyMoves) {
      const boardAfterEnemy = cloneBoard(boardAfterBot);
      simulateMove(boardAfterEnemy, em);

      const enemyGain =
        countStones(boardAfterBot, botColor) -
        countStones(boardAfterEnemy, botColor);

      // 3️⃣ BOTUN İKİNCİ HAMLESİ
      const botRecovery =
        getBestBotSecondMove(boardAfterEnemy, botColor);

      const net = enemyGain - botRecovery;

      if (net > worstEnemyGain) {
        worstEnemyGain = net;
      }
    }
  ...
  moveStone(bestMove.sr, bestMove.sc, bestMove.tr, bestMove.tc);
  switchTurn();
  render();
}
...
function simulateMove(boardCopy, move) {
  const stone = boardCopy[move.sr][move.sc];
  boardCopy[move.sr][move.sc] = null;
  boardCopy[move.tr][move.tc] = stone;

  simulateCaptures(boardCopy, move.tr, move.tc, stone.color);
}
function simulateCaptures(b, r, c, playerColor) {
  const enemy = playerColor === "mor" ? "sari" : "mor";
  ...
  for (let [dr, dc] of dirs) {
    ...
    if (
      inside(r1, c1) && inside(r2, c2) &&
      b[r1][c1]?.color === enemy &&
      b[r2][c2]?.color === playerColor
    ) {
      b[r1][c1] = null;
    }
  }
}
function getBestEnemyResponse(boardCopy, enemyColor) {
  let worstScore = 0;
  const enemyMoves = getAllPossibleMoves(enemyColor);

  for (let move of enemyMoves) {
    const tempBoard = cloneBoard(boardCopy);
    simulateMove(tempBoard, move);

    const loss = countStones(boardCopy, enemyColor) -
                 countStones(tempBoard, enemyColor);
...
function countStones(b, color) {
  return b.flat().filter(s => s?.color === color).length;
}
function opponent(color) {
  return color === "mor" ? "sari" : "mor";
}

function getBestBotSecondMove(boardCopy, botColor) {
  const moves = getAllPossibleMoves(botColor);
  let best = 0;
  ...
  for (let move of moves) {
    ...
    const gain =
      countStones(tempBoard, opponent(botColor)) -
      countStones(boardCopy, opponent(botColor));
