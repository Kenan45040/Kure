  ...
  for (let r = 0; r < SIZE; r++) {
    for (let c = 0; c < SIZE; c++) {
      ...
      for (let m of moves) {
        ...
        // 1️⃣ kaçış hamlesi uygula
        testBoard[m.r][m.c] = testBoard[r][c];
        testBoard[r][c] = null;

        // 2️⃣ mor tekrar yiyebiliyor mu?
        let morYiyor = false;
        ...
        if (morYiyor) continue; // ❌ riskli hamle

        // 3️⃣ sarı karşı yeme yapabiliyor mu?
        const sariYeme = simulateCaptures(
          testBoard,
          m.r,
          m.c,
          "sari"
        ...
        // 4️⃣ skorla
        let skor = 0;
        skor += sariYeme * 10;          // karşı yeme çok değerli
        skor += 3 - Math.abs(3 - m.r); // merkez
        skor += 3 - Math.abs(3 - m.c);
...
function akilliKacisHamlesiBul() {
  const tehditler = morTehditleriBul();
  if (tehditler.length === 0) return null;
  ...
  for (let r = 0; r < SIZE; r++) {
    for (let c = 0; c < SIZE; c++) {
      ...
      for (let m of moves) {
        ...
        // 1️⃣ mor tekrar yiyebiliyor mu?
        let morYiyor = false;
        for (let t of tehditler) {
          const eat = simulateCaptures(
            testBoard,
            t.morTo.r,
            t.morTo.c,
            "mor"
        ...
        if (morYiyor) continue; // ❌ riskli

        // 2️⃣ sarı karşı yeme yapabiliyor mu?
        let sariYeme = simulateCaptures(
          testBoard,
          m.r,
          m.c,
          "sari"
        ...
        let skor = 0;
        skor += sariYeme * 10;          // karşı yeme çok değerli
        skor += 3 - Math.abs(3 - m.r); // merkeze yakınlık
        skor += 3 - Math.abs(3 - m.c);

        if (skor > enIyiSkor) {
          enIyiSkor = skor;
          enIyi = { sr: r, sc: c, tr: m.r, tc: m.c };
        }
      }
    }
  }
